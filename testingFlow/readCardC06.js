...
-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----
`;
const keyId = "!23456789";
const appId = "com.pvcombank.ekyc.dev";
...
//     try {
//       const eidCertResult = await jose.JWK.asKey(eidCert.replace(/\\n/gm, "\n"), "pem", {
//           "kty": "RSA",
//           "alg": "RSA-OAEP-256",
//           "kid": keyId,
//           enc: "A128GCM",
...
//       }, eidCertResult).update(JSON.stringify(bodyReadCard)).final();
//       const jwe = data.toString();
//       const merchantPrivateKeyResult = await jose.JWK.asKey(merchantPrivateKey.replace(/\\n/gm, "\n"), "pem");
//       const jwsResult = await jose.JWS.createSign({
//         format: "compact",
//         fields: {kid: keyId, alg: "PS256", cty: "JWE", typ: "JOSE"}
//     }, merchantPrivateKeyResult).update(jwe).final();
...
//         },
//         data : JSON.stringify({"jws": jws})
//       };

//       axios.request(config)
//       .then((response) => {
//         // console.log(JSON.stringify(response.data));
//         fs.writeFile('./readcard.json', JSON.stringify(response.data), err => {
//             if (err) {
//               console.error(err);
//             } else {
...
//     } catch (err) {
//         console.log("Error read card", err);
//     }
// })();
...
const readCardRequestId = "bf1ca5d3-3941-49bc-b2f9-352b4d6d5e81";

(async () => {
    const bodyVerifyCard = {"request_id": readCardRequestId};
    ...
    try {
        ...
        const data = await jose.JWE.createEncrypt({
            ...
            contentAlg: "A128GCM",
            fields: {iat: parseInt((Date.now() / 1000).toString())}
        }, eidCertResult).update(JSON.stringify(bodyVerifyCard)).final();
        const jwe = data.toString();
        const merchantPrivateKeyResult = await jose.JWK.asKey(merchantPrivateKey.replace(/\\n/gm, "\n"), "pem");
        const jwsResult = await jose.JWS.createSign({
            format: "compact",
        ...
        let config = {
            method: 'post',
            maxBodyLength: Infinity,
            url: 'https://ekyc-sandbox.eidas.vn/eid/v3/verify',
            headers: {
                'appid': appId,
                'devicetype': 'mobile',
                'Content-Type': 'application/json'
        ...
        axios.request(config)
            .then((response) => {
                fs.writeFile('./verifycert.json', JSON.stringify(response.data), err => {
                    if (err) {
                      console.error(err);
                    } else {
                      // file written successfully
